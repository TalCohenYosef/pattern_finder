#!/usr/bin/env python3
"""
Script to create separate tables for each graph with pattern matching results.
Creates individual CSV files for each graph showing S numbers and 0/1 matches.
"""
...
def create_graph_table(results_dir, graph_name, total_patterns=300):
    """Create a separate table for one graph"""
    # Get pattern times
    ...
    # Find maximum S number to determine actual pattern count
    max_s = max(s_not_found) if s_not_found else total_patterns
    actual_patterns = max(total_patterns, max_s)
    
    # Create binary vector
    binary_vector = create_binary_vector(s_not_found, actual_patterns)
    
    # Create individual CSV file for this graph
    output_file = f'/home/cohent59/PROJECT_RUN_PATTERN/pattern_finder/{graph_name}_results.csv'
    ...
    with open(output_file, 'w') as f:
        ...
        f.write(f'# Total S found: {actual_patterns - len(s_not_found)}\n')
        f.write(f'# Actual pattern count: {actual_patterns}\n')
        f.write('#\n')
        
        # Data rows
        for i, found in enumerate(binary_vector, 1):
            row = [str(i), str(found)]
            f.write(','.join(row) + '\n')
    ...
    return {
        ...
        's_found_count': actual_patterns - len(s_not_found),
...
def main():
    ...
    for graph_name in graph_names:
        try:
            ...
            print(f"Created table for {graph_name}: {result['s_found_count']}/300 patterns found")
            print(f"  Saved to: {result['output_file']}")
        except Exception as e:
            print(f"Error processing {graph_name}: {e}")
    
    # Create summary file
    summary_file = '/home/cohent59/PROJECT_RUN_PATTERN/pattern_finder/graphs_summary.txt'
    with open(summary_file, 'w') as f:
        f.write("Graph Processing Summary\n")
        f.write("=" * 50 + "\n\n")
        ...
        for result in all_results:
            ...
            f.write(f"  Patterns Found: {result['s_found_count']}/{result['actual_patterns']}\n")
            f.write(f"  Patterns Not Found: {result['s_not_found_count']}/{result['actual_patterns']}\n")
